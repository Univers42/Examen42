...
int seen(unsigned int *printed,char *str)
{
    if (!CHECK_BIT_MAP(printed, (unsigned char)*str))
    {
        WRITE_CHAR(*str, 1);
        VECTOR_BIT_MAP(printed,(unsigned char)*str);
        return 1;
    }
    return 0;
}

void ft_union(char *str, char *set)
{
