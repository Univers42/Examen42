...
int ft_strcspn(char *str, char *excluded)
{
    unsigned int seen[8] = {0};
    unsigned int idx = 0;
    while (*excluded)
    {
        VECTOR_BIT_MAP(seen, *excluded);
        excluded++;
    }
    while (*str)
    {
        if (CHECK_BIT_MAP(seen,*str))
            return (idx);
        str++;
        idx++;
    }
...
int main(int argc, char **argv)
{
    ...
    char *str = argv[1];
    char *set = argv[2];
    printf("The first %i charascters in the string \"%s\""
            "are not in the string \"%s\" \n"
            , ft_strcspn(str, set),str,set);
    return (0);
}
